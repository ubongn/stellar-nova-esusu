...
export function EventFeed({ limit = 20 }: { limit?: number }) {
  ...
  return (
    <div className="card p-5">
      ...
      {!loading && error && (
        <p className="mt-4 text-sm text-gray-400 dark:text-gray-500">Unable to load activity right now.</p>
      )}

      {!loading && !error && events.length === 0 && (
        <p className="mt-4 text-sm text-gray-400 dark:text-gray-500">No on-chain activity yet.</p>
      )}
        ...
        <ul className="mt-4 max-h-80 space-y-1 overflow-y-auto pr-1">
          {events.map((e) => {
            ...
            return (
              <li
                key={e.id}
                className="flex items-start gap-3 rounded-lg px-2 py-2 transition hover:bg-gray-50 dark:hover:bg-gray-800/60"
              >
                <span className="mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400">
                  <Icon className="h-3.5 w-3.5" />
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-gray-800 dark:text-gray-100">{describe(e)}</p>
