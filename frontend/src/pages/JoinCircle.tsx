...
export function JoinCircle() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">Join a Savings Circle</h1>
      ...
      <form onSubmit={handleSearch} className="mt-6 flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-300 dark:border-gray-700 py-2.5 pl-10 pr-4 text-sm transition focus:border-brand-500 focus:ring-2 focus:ring-brand-200 dark:focus:ring-brand-500/40"
          />
        </div>
            ...
            {nameSearchResults.map((c, i) => {
              ...
              return (
                <button
                  ...
                  className="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 p-3 text-left transition hover:border-brand-300 dark:hover:border-brand-500/50 hover:bg-brand-50 dark:hover:bg-brand-500/15"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-gray-900 dark:text-white">{c.config.name}</span>
                    <span className={cx(
                      "rounded-full px-2 py-0.5 text-xs font-medium",
                      c.state === "Active" && "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300",
                      c.state === "Pending" && "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300",
                      c.state === "Completed" && "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300",
                      c.state === "Closed" && "bg-red-100 dark:bg-red-500/15 text-red-600 dark:text-red-400"
                    )}>
                      {c.state}
                    </span>
                  </div>
                  ...
                  {isMember && (
                    <span className="mt-1 inline-block text-xs font-medium text-emerald-600 dark:text-emerald-400">✓ Already joined</span>
                  )}
                </button>
              );
            })}
      ...
      {searchQuery.trim() && !/^\d+$/.test(searchQuery.trim()) && nameSearchResults.length === 0 && !loading && (
        <div className="mt-4 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
      ...
      {/* Preview */}
      {preview && (
        <div className="mt-4 rounded-2xl border border-brand-200 bg-brand-50 dark:bg-brand-500/10 p-5">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">{preview.config.name}</h2>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span
              className={cx(
                ...
                preview.state === "Active" && "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300",
                preview.state === "Pending" && "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300",
                preview.state === "Completed" && "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300"
              )}
            >
              {preview.state}
            </span>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-xl bg-white dark:bg-gray-900 p-3 text-center">
              <Users className="mx-auto h-4 w-4 text-brand-500" />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Members</p>
            ...
            <div className="rounded-xl bg-white dark:bg-gray-900 p-3 text-center">
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Per Round</p>
            ...
            <div className="rounded-xl bg-white dark:bg-gray-900 p-3 text-center">
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Rounds</p>
              <p className="text-sm font-bold">{preview.config.cycle_count}</p>
            </div>
            <div className="rounded-xl bg-white dark:bg-gray-900 p-3 text-center">
              <Crown className="mx-auto h-4 w-4 text-brand-500" />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Round</p>
          ...
          <div className="mt-4 flex items-center gap-3">
            {preview.members.includes(address ?? "") ? (
              <span className="text-sm font-medium text-emerald-700 dark:text-emerald-300">✓ You're already a member</span>
            ) : preview.members.length >= preview.config.size ? (
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Circle is full</span>
            ) : preview.state !== "Pending" ? (
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Circle already started — cannot join</span>
            ) : !address ? (
              <span className="text-sm text-amber-700 dark:text-amber-300">Connect wallet to join</span>
            ) : (
      ...
      <div className="mt-10">
        ...
        {browsing ? (
          ...
          <p className="mt-4 text-sm text-gray-400 dark:text-gray-500">
            ...
            <Link to="/create" className="text-brand-600 dark:text-brand-400 hover:underline">
              Create one!
            </Link>
          </p>
        ) : (
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {pendingCircles.map((c, i) => {
              ...
              return (
                <div
                  key={i}
                  className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-card transition hover:shadow-card-hover"
                >
                  <div className="flex items-start justify-between">
                    <h3 className="font-bold text-gray-900 dark:text-white">{c.config.name}</h3>
                    <span className="rounded-full bg-amber-50 dark:bg-amber-500/10 px-2 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-300">
                      Pending
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    {isMember ? (
                      <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">✓ Joined</span>
                    ) : isFull ? (
                      <span className="text-xs text-gray-400 dark:text-gray-500">Full</span>
                    ) : (
                      <Link
                        to={`/join`}
                        onClick={() => {
                    ...
                    <Link to={`/circle/${i}`} className="text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">
                      Details
                    </Link>
                  </div>
                </div>
